...
% CmakeConfigString - config string for the CMAKE command [Default='-DBUILD_SHARED_LIBS=OFF -DACADOS_WITH_OSQP=OFF']
    ...
    cmake_cmd = sprintf('cmake -G "Visual Studio 17 2022" -A x64 -DCMAKE_MSVC_RUNTIME_LIBRARY=MultiThreadedDLL -DCMAKE_BUILD_TYPE=Release DCMAKE_INSTALL_PREFIX="C:/Users/khanh/deps/matlab_acados/acados" "-DCMAKE_POLICY_VERSION_MINIMUM=3.5" -DACADOS_INSTALL_DIR=%%ACADOS_INSTALL_DIR%% %s ..',cmakeConfigString)
    % print cmake command
    ...
    n_jobs = 10;   % hoặc feature('numcores')

    compile_command = sprintf('cmake --build . -j%d --config Release', n_jobs);
    ...
    % 'cmake --build . -j%d --config Release --target INSTALL', n_jobs);
    ...
    % casadi
    % fprintf('Downloading casadi\n');      % prefer using the casadi installed ourself
    addpath(fullfile(acadosPath, 'interfaces', 'acados_matlab_octave'));
    % run('acados_env_variables_windows');
    run('acados_env_variables_windows_msvc');
    % check_acados_requirements(true);      % prefer using the casadi installed ourself
